...
@section('content')
    ...
    <div class="card p-3">
        ...
        @if ($room->photo)
            <img src="{{ asset('storage/' . $room->photo) }}" width="600">
        @endif
        ...
        <dl class="dl-horizontal p-2">
            ...
            <dd>{{ $room->size }}m2</dd>
            <dt>WiFi</dt>
            <dd>{{ $room->wifi ? 'WiFi' : '' }}</dd>
            <dt>Bar</dt>
            <dd>{{ $room->bar ? 'Bar' : '' }}</dd>
            <dt>Restaurant</dt>
            <dd>{{ $room->restaurant ? 'Resaurant' : '' }}</dd>
            <dt>Free parking</dt>
            <dd>{{ $room->free_parking ? 'Free Parking' : '' }}</dd>
        </dl>
    </div>
    ...
    <div class="card p-3 col-5 mt-5 mb-5">
        ...
        <form class="form_room" method="post" action="/rooms/room/booking">
            ...
            Check In <input type="date" name="dateCheckIn" class="form-control" required><br>
            Check Out <input type="date" name="dateCheckOut" class="form-control" required><br>
            ...
            <input type="tel" name="phone" class="form-control phone" required><Br>
