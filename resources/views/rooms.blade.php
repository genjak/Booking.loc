    ...
    <form method="get" action="{{ route('rooms') }}">
        <div class="row p-4">
            <div class="col-2">
                ...
                <input type="date" name="dateCheckIn" class="form-control" required
                    value="{{ !empty($dataCheckIn) ? $dataCheckIn : '' }}">
            </div>
            <div class="col-2">
                <label for="inputDate">Check-out date</label>
                <input type="date" name="dateCheckOut" class="form-control" required
                    value="{{ !empty($dataCheckOut) ? $dataCheckOut : '' }}">
            </div>
            ...
            <table class="table table-hover table-striped">
                ...
                @foreach ($rooms as $room)
                    <tr>
                        ...
                        <td>
                            @if ($room->photo)
                                <img src="{{ asset('storage/' . $room->photo) }}" width="200">
                            @endif
                        </td>
